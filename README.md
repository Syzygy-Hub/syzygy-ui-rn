...
[![Version](https://img.shields.io/badge/version-2.2.0-2F6FED)](CHANGELOG.md)
...
- **Feedback:** LoadingView, EmptyStateView, ToastView, ShimmerView, ProgressBar, PullToRefresh, ErrorStateView, SkeletonView (shape-aware shimmer placeholder), CircularProgress (determinate + indeterminate), InlineAlert (aka Banner; 4 variants using the `*Muted` tokens), Snackbar (auto-dismissing, optional action), NetworkStatusBanner (controlled/presentational — accepts `isOffline` rather than self-detecting, since real network detection needs the third-party `@react-native-community/netinfo` this library deliberately doesn't bundle), ConfirmDialog (preset confirm/cancel modal built on ModalDialog; `isDestructive` tints the confirm button)
